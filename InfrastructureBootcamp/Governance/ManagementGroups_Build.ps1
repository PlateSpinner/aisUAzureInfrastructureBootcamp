...
#>
if(!(Get-AzContext)){Write-Error "Please connect to Azure";exit}
...
foreach($MG in $MGStructure)
{
    ...
    $MgObj = Get-AzManagementGroup -GroupName $MG.ManagementGroupName -Expand -ErrorAction:SilentlyContinue
    if(!($MgObj)){
        Write-Output ("`tCreating Management Group Display Name " + $MG.ManagementGroupDisplayName + " Name " + $MG.ManagementGroupName + "...")
        $MgObj = New-AzManagementGroup -GroupName $MG.ManagementGroupName -DisplayName $MG.ManagementGroupDisplayName -ParentId $MG.ManagementGroupParent.Id        
    }
    ...
    foreach($MGSub in $MG.ChildSubscriptions){
        if(!($MgObj.Children | Where-Object -Property Name -EQ -Value $MGSub.SubscriptionId))
        {
            ...
            New-AzManagementGroupSubscription -GroupName $MG.ManagementGroupName -SubscriptionId $MGSub.SubscriptionId            
        }        
    }    
}